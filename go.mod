module serve_file

go 1.13

require (
	github.com/davyxu/cellnet v4.1.0+incompatible
	github.com/davyxu/golog v0.1.0
	github.com/davyxu/goobjfmt v0.1.0 // indirect
	github.com/davyxu/protoplus v0.1.0 // indirect
	github.com/schollz/croc/v6 v6.4.6
	golib v0.0.0-20191203014059-dd03d5084d68
)

replace golib => github.com/badforlabor/golib v0.0.0-20191203014059-dd03d5084d68
